/**
 * =====================================================================================
 * AGENT NAME: Strategist Agent
 * FILE:       07_COUNCIL_AGENTS/strategist-agent.js
...
async function analyzeStrategicFit(directiveId) {
    const directiveFilePath = path.join(DIRECTIVES_DIR, `${directiveId}.json`);
    if (!fs.existsSync(directiveFilePath)) {
        throw new Error(`Strategist: Directive file not found for ID: ${directiveId}`);
    }
    ...
    const prompt = `
        ...
        **Your Task:**
        Analyze the directive in the context of the provided file(s). Provide your strategic feedback as a concise string.
    `;

    const result = await model.generateContent(prompt);
    const feedback = (await result.response).text();

    directive.strategistFeedback = feedback.trim();
    fs.writeFileSync(directiveFilePath, JSON.stringify(directive, null, 2));
    console.log("✅  STRATEGIST: Strategic analysis complete and saved.");
}

const directiveId = process.argv[2];
if (!directiveId) {
    console.log("Usage: node 07_COUNCIL_AGENTS/strategist-agent.js <directive-id>");
    process.exit(1);
}

analyzeStrategicFit(directiveId);