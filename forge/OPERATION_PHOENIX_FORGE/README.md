...
if not os.path.exists(convert_script):
    print(f"❌ Conversion script not found at {convert_script}. Please ensure llama.cpp is cloned and built.")
    # Attempt to clone llama.cpp if not found (basic check)
    if not os.path.exists(LLAMA_CPP_PATH):
        ...
        # Note: Building llama.cpp might be required after cloning depending on the environment.
        print("Please manually build llama.cpp if necessary (e.g., run `make` in the llama.cpp directory).")
    # Re-check after potential cloning
    if not os.path.exists(convert_script):
...
if not os.path.exists(quantize_script):
     print(f"❌ Quantization script not found at {quantize_script}. Please ensure llama.cpp is built.")
     raise FileNotFoundError(f"Quantization script not found at {quantize_script}")
