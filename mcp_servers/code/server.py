...
Domain: project_sanctuary.code / sanctuary-filesystem

Refactored to use SSEServer for Gateway integration (202 Accepted + Async SSE).
Also maps to "sanctuary-filesystem" in the topology.
"""
import os
import sys
import logging
...
# Initialize
server = SSEServer("sanctuary-filesystem") # Maps to code folder
app = server.app
...
async def code_find_file(name_pattern: str, directory: str = ".") -> str:
...
async def code_list_files(directory: str = ".", pattern: str = "*", recursive: bool = True) -> str:
...
server.register_tool("code_check_tools", code_check_tools)
