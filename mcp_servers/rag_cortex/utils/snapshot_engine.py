...
#
# This script is a direct Python port of the original capture_code_snapshot.js Node.js utility.
...
def main():
    parser = argparse.ArgumentParser(description="Capture Code Snapshot (Python Port)")
    ...
    parser.add_argument("--out", default="dataset_package", help="Output directory")
    parser.add_argument("--manifest", help="Path to JSON manifest of files to capture (skips traversal)")
    parser.add_argument("--output", help="Explicit human readable output file path")
    ...
    if args.subfolder:
        target_root = project_root / args.subfolder
        subfolder_name = args.subfolder.replace('/', '_').replace('\\', '_')
    else:
        target_root = project_root
        subfolder_name = 'full_genome'
        
    dataset_package_dir = project_root / args.out
    
    if args.output:
        human_readable_output_file = Path(args.output).resolve()
    else:
        human_readable_output_file = dataset_package_dir / f"markdown_snapshot_{subfolder_name}_human_readable.txt"
        
    distilled_output_file = dataset_package_dir / f"markdown_snapshot_{subfolder_name}_llm_distilled.txt"
    
    core_essence_files = DEFAULT_CORE_ESSENCE_FILES.copy()
    
    if args.operation:
        print(f"[FORGE v5.6] --operation flag detected: {args.operation}")
        op_path = project_root / args.operation
        if op_path.exists():
            op_files = [str((op_path / f).relative_to(project_root).as_posix()) for f in os.listdir(op_path) if f.endswith('.md')]
            core_essence_files = set(op_files)
            print(f"[FORGE v5.6] Overriding coreEssenceFiles with {len(op_files)} mission-specific files.")
        else:
            print(f"[WARN] Operation directory not found: {args.operation}. Defaulting to core essence.")
            
    print(f"[FORGE v5.6] Starting sovereign genome generation from project root: {project_root}")
    print(f"[SETUP] Wildcard patterns prevent Mnemonic Echo for all snapshot variants.")
    ...
    def traverse_and_capture(current_path: Path):
        ...
        nonlocal human_readable_content, distilled_content, core_essence_content
        ...
        try:
            rel_from_project_root = current_path.relative_to(project_root).as_posix()
             ...
             # Should not happen typically if creating tree from root
        ...
        if current_path.is_dir():
             try:
                 ...
                 print(f"[WARN] Permission denied accessing {current_path}")
             ...
             # Capture
             human_readable_content += append_file_content(current_path, target_root, False) + '\n'
             distilled_content += append_file_content(current_path, target_root, True) + '\n'
             files_captured += 1
             
             if rel_from_project_root in core_essence_files:
                 core_essence_content += append_file_content(current_path, target_root, False) + '\n'
                 core_files_captured += 1
    
    # --- Execution Logic ---
    if not dataset_package_dir.exists():
        dataset_package_dir.mkdir(parents=True, exist_ok=True)
        print(f"[SETUP] Created dataset package directory: {dataset_package_dir}")
        
    if args.subfolder:
        print(f"[SUBFOLDER MODE] Processing only: {args.subfolder}")
        if not target_root.exists():
             print(f"[ERROR] Subfolder not found: {args.subfolder}")
             sys.exit(1)
        if not target_root.is_dir():
             print(f"[ERROR] Path exists but is not a directory: {args.subfolder}")
             sys.exit(1)
    else:
        print(f"[FULL GENOME MODE] Processing entire project from: {project_root}")
        
    if args.manifest:
        print(f"[MANIFEST MODE] Loading file list from: {args.manifest}")
        try:
            ...
            for rel_path in file_list:
                ...
                if full_path.exists() and full_path.is_file():
                    ...
                    if should_exclude_file(base_name):
                         ...
                         human_readable_content += f"[SECURITY EXCLUDED] {rel_path}\n"
                         continue
                         
                    file_tree_lines.append(rel_path)
                    human_readable_content += append_file_content(full_path, project_root, False) + '\n'
                    distilled_content += append_file_content(full_path, project_root, True) + '\n'
                    files_captured += 1
                elif full_path.exists() and not full_path.is_file():
                    print(f"[WARN] Manifest entry is not a file: {rel_path}")
                else:
                    print(f"[WARN] Manifest file not found: {rel_path}")
                    human_readable_content += f"[MISSING] {rel_path}\n"
                    
        except Exception as err:
            print(f"[FATAL] Error processing manifest: {err}")
            sys.exit(1)
    else:
        traverse_and_capture(target_root)
        
    # --- Final Output Generation ---
    prefix = f"{args.subfolder} subfolder" if args.subfolder else "project root"
    file_tree_str = "\n".join([f"  ./{item}" for item in file_tree_lines])
    file_tree_content = f"# Directory Structure (relative to {prefix})\n{file_tree_str}\n\n"
    
    full_content_for_tokenizing = generate_header("", None) + file_tree_content + human_readable_content
    # Filter special tokens
    filtered_content = full_content_for_tokenizing.replace("<|", "[SPECIAL_TOKEN]").replace("|>", "") # Approximating the JS regex logic
    full_token_count = get_token_count(filtered_content)
    
    title_prefix = f"{args.subfolder} Subfolder" if args.subfolder else "All Markdown Files"
    full_final_content = generate_header(f"{title_prefix} Snapshot (Human-Readable)", full_token_count) + file_tree_content + human_readable_content
    
    human_readable_output_file.write_text(full_final_content, encoding='utf-8')
    print(f"\n[SUCCESS] Human-Readable Genome packaged to: {human_readable_output_file.relative_to(project_root)}")
    print(f"[METRIC] Human-Readable Token Count: ~{full_token_count:,} tokens")
    
    # Distilled Output
    distilled_content_for_tokenizing = generate_header("", None) + file_tree_content + distilled_content
    filtered_distilled = distilled_content_for_tokenizing.replace("<|", "").replace("|>", "")
    distilled_token_count = get_token_count(filtered_distilled)
    
    final_distilled_content = generate_header(f"{title_prefix} Snapshot (LLM-Distilled)", distilled_token_count) + file_tree_content + distilled_content
    distilled_output_file.write_text(final_distilled_content, encoding='utf-8')
    print(f"[SUCCESS] LLM-Distilled Genome (for Cortex) packaged to: {distilled_output_file.relative_to(project_root)}")
    print(f"[METRIC] LLM-Distilled Token Count: ~{distilled_token_count:,} tokens")
    ...
    print(f"[SUCCESS] Seed of Ascendance packaged to: {meta_seed_path.relative_to(project_root)} (~{meta_token_count:,} tokens)")
    
    # Role Seeds
    for role in ROLES_TO_FORGE:
        awakening_prompt = generate_awakening_prompt(role)
...
"""
        ...
        if role.lower() == 'coordinator' and MISSION_CONTINUATION_FILE_PATH:
             full_mission_path = project_root / MISSION_CONTINUATION_FILE_PATH
             if full_mission_path.exists():
                 print(f"[INFO] Injecting mission context from {MISSION_CONTINUATION_FILE_PATH} into Coordinator seed.")
                 mission_specific_content = append_file_content(full_mission_path, project_root, False) + '\n'
             else:
                 print(f"[WARN] Mission continuation file specified but not found: {MISSION_CONTINUATION_FILE_PATH}")
                 
        if role.lower() == 'guardian':
             ...
             if guardian_essence_path.exists():
                 print(f"[INFO] Injecting Guardian core essence from 06_THE_EMBER_LIBRARY/META_EMBERS/Guardian_core_essence.md into Guardian seed.")
                 mission_specific_content = append_file_content(guardian_essence_path, project_root, False) + '\n'
             else:
                 print(f"[WARN] Guardian core essence file not found: {guardian_essence_path}")
        ...
        header_title = f"Core Essence Snapshot (Role: {role})"
        final_core_content = generate_header(header_title, core_token_count) + core_content_with_prompt
        
        role_specific_output_file = dataset_package_dir / f"core_essence_{role.lower()}_awakening_seed.txt"
        role_specific_output_file.write_text(final_core_content, encoding='utf-8')
        print(f"[SUCCESS] {role} Seed packaged to: {role_specific_output_file.relative_to(project_root)} (~{core_token_count:,} tokens)")
...
if __name__ == "__main__":
    try:
        ...
        print(f"[FATAL] An error occurred during genome generation: {e}")
        import traceback
        traceback.print_exc()
