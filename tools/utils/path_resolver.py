...
class PathResolver:
    ...
    def get_project_root(cls) -> str:
        ...
        2. Traverse parents looking for `.agent` or `ADRs` directories.
        ...
        # 2. Heuristic: Find '.agent' or 'ADRs' in parents (Project Sanctuary landmarks)
        current = os.path.abspath(os.getcwd())
        while True:
            if os.path.exists(os.path.join(current, ".agent")) or \
               os.path.exists(os.path.join(current, "ADRs")):
    ...
    def load_master_collection(cls) -> Dict[str, Any]:
        ...
        root = cls.get_project_root()
        path = os.path.join(root, ".agent", "learning", "object_collection.json")
