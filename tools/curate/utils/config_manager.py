...
Usage Examples:
    python tools/curate/utils/config_manager.py --help
...
def locate_rule_file(target_id: str) -> tuple[Optional[str], Optional[str]]:
    ...
    path = os.path.join(RULES_DIR, f"{target_id}_Rules.ts")
    if os.path.exists(path):
        return path, "FORM"
    
    # Check for App Menu
    path = os.path.join(RULES_DIR, f"{target_id}_Menu.ts")
    if os.path.exists(path):
        return path, "MENU"
