...
# ---------------------------------------------
# Plugin-aware Imports & Path Setup
# ---------------------------------------------
current_dir = Path(__file__).parent.resolve()
plugin_root = current_dir.parent.resolve()

# Find project root: walk up looking for .git or .agent
...
sys.path.insert(0, str(current_dir))  # Local scripts first

# Import logging: local first, then project-level
try:
    ...
    try:
        ...
        import logging
        def setup_mcp_logging(name):
            logging.basicConfig(level=logging.INFO)
            return logging.getLogger(name)

logger = setup_mcp_logging("capture_code_snapshot")

# Import snapshot_utils: local first, then project-level
try:
    from snapshot_utils import (
        ROLES_TO_FORGE, MISSION_CONTINUATION_FILE_PATH, GUARDIAN_WAKEUP_PRIMER,
        EXCLUDE_DIR_NAMES, ALWAYS_EXCLUDE_FILES, ALLOWED_EXTENSIONS,
        FILE_SEPARATOR_START, FILE_SEPARATOR_END, DEFAULT_CORE_ESSENCE_FILES,
        get_token_count, should_exclude_file, generate_header, append_file_content,
        generate_snapshot
    )
except ImportError:
    if str(project_root) not in sys.path:
