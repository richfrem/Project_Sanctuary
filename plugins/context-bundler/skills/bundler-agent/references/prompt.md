...
```bash
python plugins/context-bundler/scripts/manifest_manager.py init --target [NAME] --type [generic|tool]
```
...
```bash
# Add file
python plugins/context-bundler/scripts/manifest_manager.py add --path [path/to/file] --note "Description"

# Remove file
python plugins/context-bundler/scripts/manifest_manager.py remove --path [path/to/file]
```

### 3. Generate the Bundle
To finalize and pack the artifact:
```bash
python plugins/context-bundler/scripts/manifest_manager.py bundle --output [filename.md]
```
