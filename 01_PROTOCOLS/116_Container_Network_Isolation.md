...
During T093 (Containerize Ollama Model Service) deployment, a critical **port binding conflict** was discovered between the host-installed macOS Ollama application and the containerized `sanctuary-ollama-mcp` service. Both services bind to the same network address (`localhost:11434`), creating routing ambiguity that undermines the Federated Deployment architecture (Protocol P108).
